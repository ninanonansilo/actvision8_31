        ...
        if list_index >= len(play_list):
            ...
            for i in range(len(schedule_list)):
                if (len(schedule_list[i]) == 14): #  재생일정 미등록
                    ...
                    list_schedule.append(schedule_list[i][30:32] + ":" + schedule_list[i][32:34] + "~" + schedule_list[i][34:36] + ":" + schedule_list[i][36:38])
        ...
        if request.is_ajax():
            ...
            if (play_list_index >= len(play_list)):
                ...
                if (video_index >= len(video_name_list)):
                    ...
                    delete_blob("ynu-mcl-act", user_id + "/PLAY_LIST/" + play_list_name + "/" + video_name_list[video_index])
        ...
        if request.is_ajax():
            ...
            for i in range(len(schedule_list)): # 스케쥴 개수(재생목록 내 동영상 개수) 만큼 반복
                if len(schedule_list[i]) <= 14:  # 최초 등록 후 일정추가를 안한 영상은 패스
                    ...
                    copy_blob("ynu-mcl-act", user_id + "/PLAY_LIST/" + play_list_name +"/" + video_name_list[i],
                              "ynu-mcl-act", user_id + "/MEDIA/video/" + video_name_list[i][14:])
            ...
            for i in range(len(video_names)):  # 재생목록안 영상들 모두 업로드
                copy_blob("ynu-mcl-act", user_id + "/PLAY_LIST/" + play_list_name + "/" + video_name_list[i] , "ynu-mcl-act",
                      user_id + "/MEDIA/video/" + video_names[i])

            save_file(data)
            UPLOAD("ynu-mcl-act", user_id + "/send" ,user_id + "/JSON/TIMETABLE/" + time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()+100)))
