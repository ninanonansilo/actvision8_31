...
def save_letter(request): # 문자 설정 -> 확인 버튼 눌렀을 시 // 버켓 안 TEXT_LIST 디렉토리에 업로드하여 저장 (TIMETABLE을)
    if request != "":
        ...
        save_file(data)
        UPLOAD("ynu-mcl-act", user_id+"/send" , user_id + "/JSON/TEXT_LIST/" + now_kst1.strftime("%Y%m%d%H%M%S") + time_code + str(change[9]))
...
@csrf_exempt
def edit_letter(request): # 문자 설정 -> 확인 버튼 눌렀을 시 // 버켓 안 TEXT_LIST 디렉토리에 업로드하여 저장 (TIMETABLE을)
    if request != "":
        print("========= 시작 ===========")
        text_name_list = []
        text_list = text_list_in_bucket(user_id)
        ...
        print(change[12])
        print(change[12])
        print(text_name_list[index])
        print(text_name_list[index])
        print(text_name_list[index])
        print(text_name_list[index])
        print(text_list)
        print(text_list[index][:-len(text_name_list[index])])
        ...
        now_kst = time_now_local()  # 현재시간 받아옴
        now_kst1 = time_now()
        print(now_kst1.strftime("%Y%m%d%H%M%S"))
        print(now_kst1.strftime("%Y%m%d%H%M%S"))
        print(now_kst1.strftime("%Y%m%d%H%M%S"))
        now_kst += 15
        ...
        createDirectory(user_id)
        save_file(data)
        UPLOAD("ynu-mcl-act", user_id+"/send" , user_id + "/JSON/TEXT_LIST/" + exiting_time + str(change[9]))
...
def event_trans(request):    # 이벤트 전송 버튼 TEXT_LIST에서 선택된 TIMETABLE을 JSON/TIMETABLE로 전송
    ...
    # 추가) 기존의 타임테이블 가져와서 지나간 시간 처리########################################

    # 최신 타임테이블을 가져옴
    timetable_list = timetable_list_in_bucket()
    DOWNLOAD("ynu-mcl-act", user_id + "/JSON/TIMETABLE/" + timetable_list[-1] ,  user_id+ "/temp")
    data = read_json() # 최근 타임테이블
    ...
    UPLOAD("ynu-mcl-act", user_id + "/send",
           user_id + "/JSON/TIMETABLE/" + time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()+100)))
...
def save_img(request):
    ...
    DOWNLOAD("ynu-mcl-act", user_id + "/IMAGE/{}".format(img_list[change]), "home//static//{}.jpg".format(change))
    ...
    if request.method == 'POST':
        if request.is_ajax():
            ...
            if (delete_index >= len(img_list)):
                ...
                delete_blob("ynu-mcl-act", user_id + "/IMAGE/" + delete_name)
    ...
    if request.method == 'POST':
        if request.is_ajax():
            ...
            if (delete_index >= len(text_list)):
                ...
                delete_blob("ynu-mcl-act", user_id + "/JSON/TEXT_LIST/" + delete_name)
